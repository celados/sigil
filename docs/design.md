...
```
sigil search github                          # 跨 200+ 图标库搜索
sigil add lucide/github simple-icons/github  # 写入 icons.json
...
| sigil    | 包管理器类比      | 职责                                     |
| -------- | ----------------- | ---------------------------------------- |
| `search` | `npm search`      | 跨库搜索,人读分组文本,agent 读 `--json`  |
| `add`    | `pnpm add`        | 校验存在性后写入 `icons.json`            |
...
顶层按 set 分组——variant/prefix 是 **set 级设计决策**,挂在这里:
...
- 未注册专属 adapter 的 set 走 iconify API fallback,保住 200+ 库的
  广泛搜索;两边对同一 set 的图标命名一致(adapter 镜像 Iconify 命名),
  ref 完全可移植。
...
export interface Renderer {
	readonly id: string // 'react' | 'solid' | 'svg' | 'ripple'(规划)
...
sigil search <query> [--set lucide] [--limit 64] [--json]
sigil add <refs...> [--as <Name>]     # --as 仅允许单个 ref;DSL: set/a+b
sigil remove <refs...>
sigil list [--json]
sigil etch --output <path> [--jsx react|solid]
```
